...
def calc_dynamic_factors(logits, g, args, device):
    """
    [BUPT 多分类适配版] 计算 HAS-GNN 动态因子 G (团伙难度) 和 D (伪装难度)
    ...
    逻辑:
        1. G 和 D 的计算仅基于 "欺诈样本 (Label=1)" 的表现。
        2. 快递员(2) 和 正常人(0) 被视为 "非目标类别"，不产生结构性惩罚。
    """
    labels = g.ndata['label'].to(device)
    comp_id = g.ndata['comp_id'].to(device)
    het = g.ndata['heterophily'].to(device)
    
    # =================================================================
    # 1. 计算预测准确度 p_t (针对真实标签)
    # =================================================================
    ...
    # =================================================================
    # 2. 计算团伙难度 G (Group Difficulty)
    # =================================================================
    # 逻辑：团伙的难度 = 该团伙内所有 "欺诈节点" 的平均误差
    # (如果团伙内没有欺诈节点，或者欺诈节点都预测对了，则难度为0)
    ...
    # =================================================================
    # 3. 计算伪装难度 D (Camouflage Difficulty)
    # =================================================================
    # 公式: D = Het * (1 - pt)
    # 约束: 仅对 Fraud 生效 (通过 * is_fraud 实现)
    # 解释: 快递员(2)虽然异质性高，但 is_fraud=0，所以 D=0，不会被错误惩罚
    
    D_val = het * (1.0 - pt) * is_fraud
