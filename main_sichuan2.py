...
from utilsme import calc_has_gnn_loss

# ====================== 0. 实验记录与可视化工具类 ======================

class ExperimentManager:
    def __init__(self, base_dir="training_records_sichuan_2"):
...
def get_args():
    ...
    # === HAS-GNN Hyperparameters ===
    parser.add_argument('--lambda_I', type=float, default=1.0, help='Coefficient for Structural Importance')
    parser.add_argument('--lambda_G', type=float, default=1.0, help='Coefficient for Group Difficulty')
    parser.add_argument('--lambda_D', type=float, default=1.0, help='Coefficient for Camouflage Difficulty')
...
def train_single_layer(layer_idx, model, optimizer, g, features, labels, sample_weights, n_classes, args, stopper, exp_manager):
    ...
    for epoch in range(args.epochs):
        ...
        # 1. 计算 MLP 头的加权 Loss
        loss_node = calc_has_gnn_loss(logits, g, args, g.device)
        
        # 2. 计算 GAT 头的加权 Loss
        loss_att = calc_has_gnn_loss(logits_GAT, g, args, g.device)
        
        # 3. 组合总 Loss
        # 保持原论文的 att_loss_weight 系数逻辑
        loss = loss_node + args.att_loss_weight * loss_att
