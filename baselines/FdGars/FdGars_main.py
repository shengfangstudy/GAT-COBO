...
def FdGars_main(support: list,
    ...
    from imblearn.metrics import geometric_mean_score
    test_gmean = geometric_mean_score(label.numpy()[masks[2]].argmax(axis=1), torch.argmax(torch.from_numpy(logp.numpy()), dim=1))
    recall = recall_score(label.numpy()[masks[2]].argmax(axis=1),
                          torch.argmax(torch.from_numpy(logp.numpy()), dim=1), average='macro')
    f1=f1_score(label.numpy()[masks[2]].argmax(axis=1), torch.argmax(torch.from_numpy(logp.numpy()), dim=1), average='macro')
